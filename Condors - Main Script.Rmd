...
## Chunk 1: Species Occurrence Data Download
...
## Chunk 2: Species Occurrence Data Cleaning and Visualizing

The occurence data will be cleaned, cropped to northern South America and reprojected to ESRI:102033.
...
# write.csv(condor, "Occurence data/condor_cleaned.csv")
# write.csv(puma, "Occurence data/puma_cleaned.csv")
# write.csv(bear, "Occurence data/bear_cleaned.csv")
# 
...
# 
# # Saving the preprocessed data (we should overwrite these files later after transforming the coordinates to esri:102033)
...
## Chunk 3: Thinning data and pseudo-absences

```{r}
# Reading in data
condor <- read.csv("Occurence data/condor_final.csv")
puma <- read.csv("Occurence data/puma_final.csv")
bear <- read.csv("Occurence data/bear_final.csv")

dem <- terra::rast("Rasters/DEM.tif")

# Count how many points fall into each pixel of the dem raster
crs(condor) <- "ESRI:102033"
condor <- raster::coordinates(c("decimalLongitude", "decimalLatitude"))
condor_raster <- terra::rasterize(condor, dem, fun = "length")
...
The following chunk is not coded out properly and has not been run. I will work on it over the next few days (Manu, 19/04 11:40).
...
## use max of stream importance for rasterizing it
