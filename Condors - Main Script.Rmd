...
1.  Model Validation: Because only one data set was available, only the cross validation but not the independent validation was performed.

    1.  Create cv predictions

    2.  Calculate thresholds and make confusion matrices

    3.  Calculate and plot threshold dependent validation (Cohen's Kappa & TSS)

    4.  Calculate and plot threshold independent validation (ROC, AUC & Boyce)

2.  After the validation, the best fitted pseudo-absences (1 set per species) and models (2 per species) were selected.
