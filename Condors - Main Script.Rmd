...
library(terra)
condor <- rast("Polygon-based approach/condor_epsg4326_polygon.tif")
puma <- rast("Polygon-based approach/puma_epsg4326_polygon.tif")
bear <- rast("Polygon-based approach/bear_epsg4326_polygon.tif")

condor_reprojected <- terra::project(x = condor, y = "ESRI:102033", method = "near")
puma_reprojected <- terra::project(x = puma, y = "ESRI:102033", method = "near")
bear_reprojected <- terra::project(x = bear, y = "ESRI:102033", method = "near")

condor_cropped <- crop(condor_reprojected, area)
condor_cropped <- mask(condor_reprojected, area)
plot(condor_cropped)

puma_cropped <- crop(puma_reprojected, area)
puma_cropped <- mask(puma_reprojected, area)
plot(puma_cropped)

bear_cropped <- crop(bear_reprojected, area)
bear_cropped <- mask(bear_reprojected, area)
plot(bear_cropped)
