...
**To run the script, paths need to be adapted to where the data is stored. Relevant lines are marked with "\### CHANGE PATH".**

Most data has been run through the repository (<https://github.com/Manuel-Weber-ETH/Condor-Landscape-Modelling.git>), however when heavy rasters were involved, they were loaded from google drive (<https://drive.google.com/drive/folders/1RhKo5il5ji5PHPXgBQtaA75J20gKIQif>) or the local machine.
...
Next, 10 000 environmentally stratified pseudo-absences are generated. This entails to aggregate the 1x1km resolution DEM to a 10x10km resolution to keep computing times reasonable. Furthermore, the stratification of the topography was implemented in classes of 100 meters. We did not use this set of pseudo absences in the modeling process.
...
setwd("Condor-Landscape-Modelling") ### CHANGE PATH
library(sf)
area <- read_sf("Study area shapefile/study area shapefile.shp")
area <- area[1]
...
lakes <- read_sf("DRIVE/Hydro_LAKES/Hydro_LAKES/sa_lakes.shp")%>% ### CHANGE PATH
  st_transform(st_crs(area))
rivers <- read_sf("DRIVE/Hydro_RIVERS/HydroRIVERS_v10_sa.shp")%>% ### CHANGE PATH
  st_transform(st_crs(area))
...
water<-rast("DRIVE/sa_water_1km_aea.tif") ### CHANGE PATH
...
writeRaster(water_focal, "water_focal.tif", overwrite=TRUE)


### Read in the data for all predictors  ### CHANGE PATH
## DEM
dem <- rast("Rasters/DEM.tif")
## Mean annual air temperature (bio1)
temp_mean_annual <- rast('CHELSA_bio1_1981-2010_V.2.1.tif')
## Mean diurnal air temperature range (bio2)
temp_mean_diurnal <- rast('CHELSA_bio2_1981-2010_V.2.1.tif')
## Annual range of air temperature (bio7)
temp_range <- rast('CHELSA_bio7_1981-2010_V.2.1.tif')
## Annual Precipitation amount (bio12)
prec_annual <- rast('CHELSA_bio12_1981-2010_V.2.1.tif')
## Precipitation amount of the wettest month (bio13)
prec_wettest_month <- rast('CHELSA_bio13_1981-2010_V.2.1.tif')
## Precipitation amount of the driest month (bio14)
prec_driest_month <- rast('CHELSA_bio14_1981-2010_V.2.1.tif')
## Tree cover
...
predictors <- rast("predictors.tif") ### CHANGE PATH
...
# Load fitted models
fitted_model <- readRDS("fitted_models.rds") ### CHANGE PATH
...
used_preds_condor <- rast("predictors_condor_new.tif") ### CHANGE PATH
...
# Get subset of predictors for each species, which is stored locally
used_preds_puma <- rast("predictors_puma_new.tif") ### CHANGE PATH
...
# Get subset of predictors for each species, which is stored locally
used_preds_bear <- rast("predictors_bear_new.tif") ### CHANGE PATH
...
#load models
fitted_model<-readRDS("Condor-Landscape-Modelling/fitted_models.rds") ### CHANGE PATH
...
predictors_condor <- rast("predictors_condor_new.tif") ### CHANGE PATH
...
predictors_puma <- rast("predictors_puma_new.tif") ### CHANGE PATH

### Load fitted models
# Load all fitted models
fitted_models <- readRDS("fitted_models.rds") ### CHANGE PATH
summary(fitted_models)
...
predictors_bear <- rast("predictors_bear_new.tif") ### CHANGE PATH
...
library(terra)
library(sf)
area <- read_sf("shapefiles/study area shapefile.shp")[1]  ### CHANGE PATH

### Condor
condor <- rast("Condor random gbm and random rf.tif")  ### CHANGE PATH
...
setwd("Condor-Landscape-Modelling/Occurrence data") ### CHANGE PATH
...
setwd("Condor-Landscape-Modelling/Polygon-based approach") ### CHANGE PATH
...
input_dir <- "Condor-Landscape-Modelling/Polygon-based approach"  ### CHANGE PATH
...
#### From here, the work has been transferred to QGIS 3
