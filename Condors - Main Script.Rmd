...
We then cropped all layer to the study area, reprojected to the DEM raster, and aligned the rasters.

```{r}
## Read in downloaded rasters
...
############################################################################################
#The code above has been run, the file is stored on the google drive, please don't run again but access the files directly (Justine 5/5; 11:34). Tanja added the water bodies to the predictor raster, so no need to access the water bodies from the google drive. 
############################################################################################
```

In the following part, we tested for autocorrelations between the predictors with the threshold set at 0.7.
...
# 
# 
# saveRDS(modli, file = "fitted_models.rds")
#
############################################################################################
...
## Chunk 6: Model Evaluation, Projection and Ensemble (Justine & Tanja)
...
1.  Model Validation: Because only one data set was available, only the cross validation but not the independent validation was performed. There following steps were done:
    ...
    3.  Calculate threshold dependent validation (Cohen's Kappa & TSS). The following plots created:![](images/Kappa "Cohen's Kappa"){width="316"}![](images/TSS "TSS"){width="316"}

    4.  Calculate threshold independent validation (ROC, AUC & Boyce). The following plots created:

        ![](images/ROC "ROC"){width="316"}![](images/AUC "AUC"){width="316"}

        ![](images/Boyce "Boyce"){width="316"}![](images/Boyce%20Index "Boyce Index"){width="316"}
...
save(cv_thres_list, file="C:/Users/justi/OneDrive/Studium/Umweltnaturwissenschaften/Landscape Modelling/Condor/Condor-Landscape-Modelling/cv_thres_list.rda")
...
The following three basic visualizations are stored in the same folder and display the occurrence data (in red) over the generated range (in green): **(Do not use them for the presentation, I will produce better one's in QGIS)**
...
## Chunk 8: Down-scaling to Colombia; Comparison with PA-network (Manuel)
...
## Reading in PA network ### Possibly too large, I outsource to QGIS here
setwd("C:/0_Documents/10_ETH/12_Mon 13-16_CHN E42_Landscape Modelling of Biodiversity/Project/QGIS/Protected_areas_poly")
PAs <- readOGR()

```

![](Species%20Richness%20and%20PAs%20Colombia.jpeg)
